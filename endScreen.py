...
avL = (screenSize[1] - 2*space3 - 4*space1 - fontsize1 - winnerBannerRect.bottom - space2)/4
avatarSize2 = (avL, avL)

font = pg.font.Font('freesansbold.ttf', fontsize1)
titleText = ["Joueur", "Score", "Cartes en main"]
titles = [font.render(t, True, blue, backgroundColor) for t in titleText]
...
class endScreen:
	def __init__(self, screen, playerList):
		...
		self.scores = [font.render(str(playerList[i][1]), True, darkBlue, backgroundColor) for i in order]
		self.scoreRect = [i.get_rect() for i in self.scores]
		self.nCartes = [font.render(str(playerList[i][2]), True, darkBlue, backgroundColor) for i in order]
